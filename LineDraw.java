...
import java.awt.*; //載入java.awt的所有物件和方法。包含用於創建使用者介面和繪製圖形圖像的全部類別。
import javax.swing.*; //載入javax.swing的所有物件和方法。提供一組「輕量級」的JAVA元件，盡量讓這些元件在所有平臺上的工作方式都相同。
import java.awt.event.*; //載入java.awt.event的所有物件和方法。提供處理由 AWT 元件所觸發的各類別事件的介面和類別。
 
public class LineDraw extends JFrame implements ActionListener{ //宣告LineDraw類別，並將其繼承JFrame物件。實作ActionListener(可用","隔開不同實作)，接收操作JFrame的事件。
